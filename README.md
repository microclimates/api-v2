...
**503** *Bad Gateway* - The cloud servers are having a problem connecting with the site server.  
...
## View outstanding invitations [GET /api/v2/invites]

This is used to view invitations created by the requester that have not been accepted.
...
account if necessary, and connects the user into the site with the proper security credentials.
...
+ siteName: `Indoor Farm North` (string, readonly)
    Site name. Provided for convenience on GET.
+ invitedBy: `Mary Maker` (string, readonly)
